...
class ShiftScheduleService
{
    ...
    {
        Log::info("🟢 determinePunchType() START - Employee ID: {$employeeId}, Punch ID: {$punchId}, Punch Time: {$punchTime}");
        // Fetch shift schedule
        $schedule = $this->getShiftScheduleForEmployee($employeeId);
        if (!$schedule) {
            Log::warning("⚠️ No schedule found for Employee ID: {$employeeId}, Punch ID: {$punchId}. Defaulting to heuristic rules.");
            $punchType = $this->heuristicPunchTypeAssignment($employeeId, $punchTime);
            Log::info("✅ Punch ID: {$punchId} assigned using **Heuristic Method** -> Punch Type: " . ($punchType ?? 'NULL'));
            return $punchType;
        }
        Log::info("ℹ️ Using Shift Schedule for Employee ID: {$employeeId}, Shift Start: {$schedule->start_time}, Shift End: {$schedule->end_time}");
        // Train ML model based on past data
        Log::info("🚀 Training ML Model for Punch Type Prediction...");
        $this->trainModel();
        // Predict punch type using ML
        $predictedType = $this->predictPunchType($punchTime);
        if ($predictedType) {
            Log::info("✅ Punch ID: {$punchId} assigned using **ML Model Prediction** -> Predicted Punch Type: {$predictedType}");
            return $predictedType;
        } else {
            Log::warning("⚠️ ML Prediction failed for Punch ID: {$punchId}. Falling back to heuristic method.");
        }
        // Fallback to heuristic if ML prediction fails
        $fallbackType = $this->heuristicPunchTypeAssignment($employeeId, $punchTime);
        Log::info("✅ Punch ID: {$punchId} assigned using **Heuristic Fallback** -> Final Punch Type: " . ($fallbackType ?? 'NULL'));
        Log::info("🔴 determinePunchType() END - Employee ID: {$employeeId}, Punch ID: {$punchId}");
        return $fallbackType;
    }
    /**
    ...
    {
        Log::info("trainModel() was called."); // Step 1: Check if method is called
        $attendanceData = Attendance::whereNotNull('punch_type_id')
            ->select('employee_id', 'punch_time', 'punch_type_id')
            ->get();
        Log::info("Fetched " . $attendanceData->count() . " attendance records."); // Step 2: Confirm record fetch
        ...
        Log::info("ML Model trained with " . count($samples) . " attendance records."); // Step 3: Confirm ML training
    }
    /**
     ...
     */
    public function processStaleRecords(): void
    {
        Log::info("Starting stale record processing...");
        // Find stale records (Punch Type is NULL and older than 1 day)
        $staleRecords = Attendance::whereNull('punch_type_id')
            ->where('punch_time', '<', now()->subDay())
            ->get();
        Log::info("Found " . $staleRecords->count() . " stale attendance records."); // Step 1: Check how many stale records exist
