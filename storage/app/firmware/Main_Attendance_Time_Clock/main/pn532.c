 ...
 * This is a PN532 Driver for the ESP32 family and IDF 5.3 for NXP's PN532
 * NFC/13.56MHz RFID Transceiver. This component is inspired the Adafruit
 * library.
 */
...
const uint8_t pn532response_firmwarevers[] = {0x00, 0xFF, 0x06,
											  0xFA, 0xD5, 0x03};
...
esp_err_t pn532_get_firmware_version(pn532_io_handle_t io_handle,
									 uint32_t *fw_version) {
	esp_err_t err;
	...
	err = pn532_send_command_wait_ack(io_handle, pn532_packetbuffer, 1,
									  PN532_WRITE_TIMEOUT);
	if (ESP_OK != err) {
		return err;
	}
	...
	// read data packet
	err =
		pn532_read_data(io_handle, pn532_packetbuffer, 12, PN532_READ_TIMEOUT);
	if (ESP_OK != err)
		return err;

	// check some basic stuff
	if (0 != memcmp(pn532_packetbuffer + 1, pn532response_firmwarevers,
					sizeof(pn532response_firmwarevers))) {
#ifdef CONFIG_PN532DEBUG
		ESP_LOGD(
			TAG,
			"pn532_get_firmware_version(): get firmware response invalid!");
#endif
		return ESP_FAIL;
	}
...
esp_err_t pn532_set_passive_activation_retries(pn532_io_handle_t io_handle,
											   uint8_t maxRetries) {
	...
	ESP_LOGD(TAG,
			 "pn532_set_passive_activation_retries(): Setting "
			 "MxRtyPassiveActivation to %d",
			 maxRetries);
#endif

	return pn532_send_command_wait_ack(io_handle, pn532_packetbuffer, 5,
									   PN532_WRITE_TIMEOUT);
}
...
esp_err_t pn532_read_passive_target_id(pn532_io_handle_t io_handle,
	...
	pn532_packetbuffer[1] =
		1; // currently only support one card (PN532 can handle two cards)
	pn532_packetbuffer[2] = baud_rate_and_card_type;

	esp_err_t err = pn532_send_command_wait_ack(io_handle, pn532_packetbuffer,
												3, PN532_WRITE_TIMEOUT);
	if (ESP_OK != err) {
#ifdef CONFIG_PN532DEBUG
...
esp_err_t pn532_in_data_exchange(pn532_io_handle_t io_handle,
								 ...
								 uint8_t send_buffer_length, uint8_t *response,
	...
	err = pn532_read_data(io_handle, pn532_packetbuffer,
						  sizeof(pn532_packetbuffer), PN532_READ_TIMEOUT);
	if (ESP_OK != err)
		return err;
	...
	if (pn532_packetbuffer[0] == 0 && pn532_packetbuffer[1] == 0 &&
...
#ifdef CONFIG_PN532DEBUG
			ESP_LOGD(TAG, "Length check invalid 0x%02X 0x%02X", length,
		...
		if (pn532_packetbuffer[5] == PN532_PN532TOHOST &&
			...
			*response_length = length;
			...
			ESP_LOGD(TAG, "Don't know how to handle this command: 0x%.2X",
...
esp_err_t pn532_in_list_passive_target(pn532_io_handle_t io_handle) {
	...
	if (pn532_packetbuffer[0] == 0 && pn532_packetbuffer[1] == 0 &&
		...
		if (0 != ((pn532_packetbuffer[4] + length) & 0xFF)) {
#ifdef CONFIG_PN532DEBUG
			ESP_LOGD(TAG, "Length check invalid 0x%.2X 0x%.2X", length,
					 pn532_packetbuffer[4]);
#endif
			return ESP_FAIL;
		}
...
#endif
				ESP_LOGI(TAG, "Number of tags inListed: %d",
...
esp_err_t ntag2xx_authenticate(pn532_io_handle_t io_handle, uint8_t page,
							   uint8_t *key, uint8_t *uid, uint8_t uid_length) {
	pn532_packetbuffer[0] = PN532_COMMAND_INDATAEXCHANGE;
	pn532_packetbuffer[1] = 1;
	...
	esp_err_t err = pn532_send_command_wait_ack(
		io_handle, pn532_packetbuffer, 10 + uid_length, PN532_WRITE_TIMEOUT);

	return err;
}

esp_err_t ntag2xx_read_page(pn532_io_handle_t io_handle, uint8_t page,
							uint8_t *buffer, size_t read_len) {
	...
	err =
		pn532_read_data(io_handle, pn532_packetbuffer, 26, PN532_READ_TIMEOUT);
	if (err != ESP_OK)
		return err;
...
#ifdef CONFIG_MIFAREDEBUG
		ESP_LOGD(TAG, "Status byte indicates an error: 0x%02x",
				 pn532_packetbuffer[7]);
#endif
		return ESP_FAIL;
	}
...
esp_err_t ntag2xx_write_page(pn532_io_handle_t io_handle, uint8_t page,
							 const uint8_t *data) {
	// TAG Type       PAGES   USER START    USER STOP
	// --------       -----   ----------    ---------
	// NTAG 203       42      4             39
