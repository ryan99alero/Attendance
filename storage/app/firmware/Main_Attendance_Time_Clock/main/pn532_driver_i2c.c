...
esp_err_t pn532_new_driver_i2c(gpio_num_t sda, gpio_num_t scl, gpio_num_t reset,
							   gpio_num_t irq, i2c_port_num_t i2c_port_number,
	...
	pn532_i2c_driver_config *dev_config = heap_caps_calloc(
		1, sizeof(pn532_i2c_driver_config), MALLOC_CAP_DEFAULT);
	if (dev_config == NULL) {
...
esp_err_t pn532_init_io(pn532_io_handle_t io_handle) {
	...
	pn532_i2c_driver_config *driver_config =
		(pn532_i2c_driver_config *)io_handle->driver_data;
	...
	if (driver_config->scl != GPIO_NUM_NC &&
		driver_config->sda != GPIO_NUM_NC) {
		...
		if (i2c_new_master_bus(&conf, &driver_config->i2c_bus_handle) !=
			ESP_OK) {
		...
		if (i2c_master_get_bus_handle(driver_config->i2c_port_number,
									  &driver_config->i2c_bus_handle) !=
			ESP_OK) {
	...
	dev_cfg.device_address =
		PN532_I2C_RAW_ADDRESS; // 7-bit address without RW flag
	dev_cfg.scl_speed_hz = 100000;
	dev_cfg.scl_wait_us = 200000;

	if (i2c_master_bus_add_device(driver_config->i2c_bus_handle, &dev_cfg,
								  &driver_config->i2c_dev_handle) != ESP_OK) {
...
esp_err_t pn532_is_ready(pn532_io_handle_t io_handle) {
	...
	pn532_i2c_driver_config *driver_config =
		(pn532_i2c_driver_config *)io_handle->driver_data;
	esp_err_t result =
		i2c_master_receive(driver_config->i2c_dev_handle, &status, 1, 10);
...
esp_err_t pn532_read(pn532_io_handle_t io_handle, uint8_t *read_buffer,
	...
	TickType_t timeout_ticks =
		(xfer_timeout_ms > 0) ? pdMS_TO_TICKS(xfer_timeout_ms) : portMAX_DELAY;
	...
	while (!is_ready && elapsed_ticks < timeout_ticks) {
		result = i2c_master_receive(driver_config->i2c_dev_handle, rx_buffer,
									read_size + 1, read_timeout);
...
esp_err_t pn532_write(pn532_io_handle_t io_handle, const uint8_t *write_buffer,
	...
	return i2c_master_transmit(driver_config->i2c_dev_handle,
