...
const int32_t baud_config_table[] = {9600,	 19200,	 38400,	 57600,	 115200,
									 230400, 460800, 921600, 1288000};

static const uint8_t get_firmware_frame[] = {0x00, 0xFF, 0x02, 0xFE,
											 0xD4, 0x02, 0x2A};
static const uint8_t set_serial_baud_rate_resp_frame[] = {
	0x00, 0x00, 0xFF, 0x02, 0xFE, 0xD5, 0x11, 0x1A, 0x00};
...
static esp_err_t pn532_read(pn532_io_handle_t io_handle, uint8_t *read_buffer,
							size_t read_size, int xfer_timeout_ms);
static esp_err_t pn532_write(pn532_io_handle_t io_handle,
...
esp_err_t pn532_new_driver_hsu(gpio_num_t uart_rx, gpio_num_t uart_tx,
							   gpio_num_t reset, gpio_num_t irq,
							   uart_port_t uart_port, int32_t baudrate,
							   pn532_io_handle_t io_handle) {
	if (io_handle == NULL)
		return ESP_ERR_INVALID_ARG;

	if (uart_rx == GPIO_NUM_NC || uart_tx == GPIO_NUM_NC ||
		uart_port >= UART_NUM_MAX)
		return ESP_ERR_INVALID_ARG;

	pn532_hsu_driver_config *dev_config = heap_caps_calloc(
		1, sizeof(pn532_hsu_driver_config), MALLOC_CAP_DEFAULT);
	if (dev_config == NULL) {
	...
	for (int n = 0;
		 n < (sizeof(baud_config_table) / sizeof(baud_config_table[0])); ++n) {
	...
	if (baud_config_table[dev_config->uart_baud_wanted] != baudrate) {
		ESP_LOGW(TAG,
				 "pn532_new_driver_hsu(): Unsupported baud rate %ld -> using "
...
static void pn532_release_driver(pn532_io_handle_t io_handle) {
	...
	pn532_hsu_driver_config *driver_config =
		(pn532_hsu_driver_config *)io_handle->driver_data;
...
esp_err_t pn532_init_io(pn532_io_handle_t io_handle) {
	...
	if (ESP_OK !=
		uart_driver_install(driver_config->uart_port, 256, 256, 0, NULL, 0)) {
	...
	if (ESP_OK != uart_set_pin(driver_config->uart_port, driver_config->uart_tx,
							   driver_config->uart_rx, GPIO_NUM_NC,
							   GPIO_NUM_NC)) {
		ESP_LOGE(TAG, "uart_set_pin() failed");
		return ESP_FAIL;
	}

	ESP_LOGD(TAG, "pn532_init_io(): check baud rate %ld ...",
			 baud_config_table[driver_config->uart_baud_used]);
	esp_err_t err = pn532_write(io_handle, get_firmware_frame,
								sizeof(get_firmware_frame), 100);
	if (err != ESP_OK) {
	...
	ESP_LOGD(TAG, "pn532_init_io(): try to use configured baud rate %ld ...",
			 baud_config_table[driver_config->uart_baud_wanted]);
	err = uart_set_baudrate(driver_config->uart_port,
							baud_config_table[driver_config->uart_baud_wanted]);
	if (err != ESP_OK)
		return err;
	driver_config->uart_baud_used = driver_config->uart_baud_wanted;

	err = pn532_write(io_handle, get_firmware_frame, sizeof(get_firmware_frame),
					  100);
	if (err != ESP_OK) {
		ESP_LOGD(TAG, "pn532_init_io(): failed to send get firmware frame on "
					  "configured baudrate");
	}
...
esp_err_t pn532_init_extra(pn532_io_handle_t io_handle) {
	...
	if (driver_config->uart_baud_used != driver_config->uart_baud_wanted) {
		...
		// read response
		err = pn532_read(io_handle, buf,
						 sizeof(set_serial_baud_rate_resp_frame), 100);
		if (err != ESP_OK)
			return err;

		ESP_LOGD(TAG, "pn532_init_extra() received response frame:");
		ESP_LOG_BUFFER_HEXDUMP(
			TAG, buf, sizeof(set_serial_baud_rate_resp_frame), ESP_LOG_DEBUG);
		if (0 != memcmp(buf, set_serial_baud_rate_resp_frame,
						sizeof(set_serial_baud_rate_resp_frame)))
			return ESP_FAIL;
...
esp_err_t pn532_wakeup(pn532_hsu_driver_config *driver_config) {
	const static uint8_t wakeup_frame[] = {0x55, 0x55, 0x55, 0x00, 0x00,
										   0x00, 0x00, 0x00, 0x00, 0x00};

	int result = uart_write_bytes(driver_config->uart_port, wakeup_frame,
								  sizeof(wakeup_frame));
...
esp_err_t pn532_read(pn532_io_handle_t io_handle, uint8_t *read_buffer,
					 size_t read_size, int xfer_timeout_ms) {
	...
	TickType_t timeout_ticks = (xfer_timeout_ms > 0)
								   ? pdMS_TO_TICKS(xfer_timeout_ms) + 1
								   : portMAX_DELAY;

	int rx_bytes = 0;
	TickType_t elapsed_ticks = 0;

	rx_bytes = uart_read_bytes(driver_config->uart_port, read_buffer, 6,
							   timeout_ticks);
	if (rx_bytes != 6) {
		if (rx_bytes < 0)
			return ESP_FAIL;
	...
	rx_bytes = uart_read_bytes(driver_config->uart_port, read_buffer + 6,
							   bytes_to_read, timeout_ticks - elapsed_ticks);
...
esp_err_t pn532_write(pn532_io_handle_t io_handle, const uint8_t *write_buffer,
	...
	// Frame
	result =
		uart_write_bytes(driver_config->uart_port, write_buffer, write_size);
