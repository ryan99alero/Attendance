...
esp_err_t pn532_new_driver_spi(gpio_num_t miso, gpio_num_t mosi, gpio_num_t sck,
							   gpio_num_t cs, gpio_num_t reset, gpio_num_t irq,
							   spi_host_device_t spi_host,
							   int32_t clock_frequency,
	...
	pn532_spi_driver_config *dev_config = heap_caps_calloc(
		1, sizeof(pn532_spi_driver_config), MALLOC_CAP_DEFAULT);
	if (dev_config == NULL) {
...
void spi_pre_cb(spi_transaction_t *trans) {
	pn532_spi_driver_config *driver_config =
		(pn532_spi_driver_config *)trans->user;
...
esp_err_t pn532_init_io(pn532_io_handle_t io_handle) {
	...
	pn532_spi_driver_config *driver_config =
		(pn532_spi_driver_config *)io_handle->driver_data;
	...
	if (driver_config->sck != GPIO_NUM_NC &&
		...
		err = spi_bus_initialize(driver_config->spi_host, &bus_config,
								 SPI_DMA_CH_AUTO);
		...
		if (err != ESP_OK) {
			if (err == ESP_ERR_INVALID_STATE) {
				ESP_LOGW(
					TAG,
					"Failed to initialize SPI bus, maybe already initialzed");
			} else {
	...
	err = spi_bus_add_device(driver_config->spi_host, &dev_config,
							 &driver_config->spi_handle);
...
esp_err_t pn532_is_ready(pn532_io_handle_t io_handle) {
	...
	esp_err_t result = spi_device_polling_transmit(
		driver_config->spi_handle, &(spi_transaction_t){
									   .cmd = OP_READ_STATUS,
									   .rxlength = 8,
									   .rx_buffer = &status,
