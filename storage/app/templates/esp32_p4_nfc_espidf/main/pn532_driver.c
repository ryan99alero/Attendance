...
esp_err_t pn532_init(pn532_io_handle_t io_handle) {
	...
	if (io_handle->irq != GPIO_NUM_NC) {
		...
		gpio_isr_handler_add(io_handle->irq, pn532_irq_handler,
							 (void *)io_handle);
...
esp_err_t pn532_write_command(pn532_io_handle_t io_handle, const uint8_t *cmd,
							  uint8_t cmdlen, int timeout) {
...
esp_err_t pn532_read_data(pn532_io_handle_t io_handle, uint8_t *buffer,
						  uint8_t length, int32_t timeout) {
	...
	esp_err_t res =
		io_handle->pn532_read(io_handle, local_buffer, length, timeout);
	if (res != ESP_OK) {
		return res;
	}
...
#endif
static esp_err_t pn532_poll_ready(pn532_io_handle_t io_handle,
								  int32_t timeout) {
	TickType_t start_ticks = xTaskGetTickCount();
	TickType_t timeout_ticks =
		(timeout > 0) ? pdMS_TO_TICKS(timeout) : portMAX_DELAY;
...
esp_err_t pn532_SAM_config(pn532_io_handle_t io_handle) {
	...
	static const uint8_t sam_config_frame[] = {
		0x14, 0x01, 0x00, 0x01}; // normal mode and use IRQ pin

	result = pn532_send_command_wait_ack(io_handle, sam_config_frame,
										 sizeof(sam_config_frame), 1000);
	if (ESP_OK != result)
		return result;
	...
	// read data packet
	result =
		pn532_read_data(io_handle, response_buffer, 10, PN532_READ_TIMEOUT);
	if (ESP_OK != result)
		return result;
...
esp_err_t pn532_send_command_wait_ack(pn532_io_handle_t io_handle,
									  const uint8_t *cmd, uint8_t cmd_length,
									  int32_t timeout) {
	esp_err_t result;

	if (io_handle == NULL || io_handle->driver_data == NULL || cmd == NULL ||
		cmd_length == 0) {
		return ESP_ERR_INVALID_ARG;
	}
...
esp_err_t pn532_read_ack(pn532_io_handle_t io_handle) {
	...
	result = pn532_read_data(io_handle, ack_buffer, sizeof(ACK_FRAME),
							 PN532_READ_TIMEOUT);
	if (result != ESP_OK)
		return result;
