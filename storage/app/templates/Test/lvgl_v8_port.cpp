    ...
    if (lv_disp_flush_is_last(drv)) {
        ...
        if (drv->full_refresh) {
            ...
            if (!ulTaskNotifyTake(pdTRUE, pdMS_TO_TICKS(200))) {
                ESP_LOGW("LvPort", "Frame buffer transmission timeout (1)");
            }
            ...
            if (probe_result == FLUSH_PROBE_FULL_COPY) {
                ...
                ulTaskNotifyValueClear(NULL, ULONG_MAX);
                if (!ulTaskNotifyTake(pdTRUE, pdMS_TO_TICKS(200))) {
                    ESP_LOGW("LvPort", "Frame buffer transmission timeout (2)");
                }
...
static void flush_callback(lv_disp_drv_t *drv, const lv_area_t *area, lv_color_t *color_map)
{
    ...
    if (lv_disp_flush_is_last(drv)) {
        ...
        ulTaskNotifyValueClear(NULL, ULONG_MAX);
        if (!ulTaskNotifyTake(pdTRUE, pdMS_TO_TICKS(200))) {
            ESP_LOGW("LvPort", "Frame buffer transmission timeout (3)");
        }
    }
...
static void flush_callback(lv_disp_drv_t *drv, const lv_area_t *area, lv_color_t *color_map)
{
    ...
    ulTaskNotifyValueClear(NULL, ULONG_MAX);
    if (!ulTaskNotifyTake(pdTRUE, pdMS_TO_TICKS(200))) {
        ESP_LOGW("LvPort", "Frame buffer transmission timeout (4)");
    }
...
IRAM_ATTR bool onDrawBitmapFinishCallback(void *user_data)
{
    ...
    // Notify the LVGL task that frame buffer transmission is complete
    if (lvgl_task_handle != nullptr) {
        BaseType_t xHigherPriorityTaskWoken = pdFALSE;
        vTaskNotifyGiveFromISR(lvgl_task_handle, &xHigherPriorityTaskWoken);
...
bool lvgl_port_init(LCD *lcd, Touch *tp)
{
    ...
    // TEMPORARILY DISABLE CALLBACK TO AVOID DEADLOCK
    // For non-RGB LCD, need to notify LVGL that the buffer is ready when the refresh is finished
    if (false) { // Disabled: bus_type != ESP_PANEL_BUS_TYPE_RGB
        ...
        ESP_UTILS_LOGW("Callback disabled to avoid deadlock - display may have issues");
    }
